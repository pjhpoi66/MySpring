...
/*
 * modified 1.2.4 touchSwipe - jQuery Plugin
 * http://plugins.jquery.com/project/touchSwipe
 * http://labs.skinkers.com/touchSwipe/
 * 
 * Copyright (c) 2010 Matt Bryson (www.skinkers.com) Dual licensed under the MIT
 * or GPL Version 2 licenses.
 * 
 * $version: 1.2.4
 * 
 * Changelog $Date: 2010-12-12 (Wed, 12 Dec 2010) $ $version: 1.0.0 $version:
 * 1.0.1 - removed multibyte comments
 * 
 * $Date: 2011-21-02 (Mon, 21 Feb 2011) $ $version: 1.1.0 - added
 * allowPageScroll property to allow swiping and scrolling of page - changed
 * handler signatures so one handler can be used for multiple events $Date:
 * 2011-23-02 (Wed, 23 Feb 2011) $ $version: 1.2.0 - added click handler. This
 * is fired if the user simply clicks and does not swipe. The event object and
 * click target are passed to handler. - If you use the
 * http://code.google.com/p/jquery-ui-for-ipad-and-iphone/ plugin, you can also
 * assign jQuery mouse events to children of a touchSwipe object. $version:
 * 1.2.1 - removed console log!
 * 
 * $version: 1.2.2 - Fixed bug where scope was not preserved in callback
 * methods.
 * 
 * $Date: 2011-28-04 (Thurs, 28 April 2011) $ $version: 1.2.4 - Changed licence
 * terms to be MIT or GPL inline with jQuery. Added check for support of touch
 * events to stop non compatible browsers erroring.
 * 
 * A jQuery plugin to capture left, right, up and down swipes on touch devices.
 * You can capture 2 finger or 1 finger swipes, set the threshold and define
 * either a catch all handler, or individual direction handlers. Options: swipe
 * Function A catch all handler that is triggered for all swipe directions.
 * Handler is passed 3 arguments, the original event object, the direction of
 * the swipe : "left", "right", "up", "down" and the distance of the swipe.
 * swipeLeft Function A handler that is triggered for "left" swipes. Handler is
 * passed 3 arguments, the original event object, the direction of the swipe :
 * "left", "right", "up", "down" and the distance of the swipe. swipeRight
 * Function A handler that is triggered for "right" swipes. Handler is passed 3
 * arguments, the original event object, the direction of the swipe : "left",
 * "right", "up", "down" and the distance of the swipe. swipeUp Function A
 * handler that is triggered for "up" swipes. Handler is passed 3 arguments, the
 * original event object, the direction of the swipe : "left", "right", "up",
 * "down" and the distance of the swipe. swipeDown Function A handler that is
 * triggered for "down" swipes. Handler is passed 3 arguments, the original
 * event object, the direction of the swipe : "left", "right", "up", "down" and
 * the distance of the swipe. swipeStatus Function A handler triggered for every
 * phase of the swipe. Handler is passed 4 arguments: event : The original event
 * object, phase:The current swipe face, either "start�, "move�, "end� or
 * "cancel�. direction : The swipe direction, either "up�, "down�, "left " or
 * "right�.distance : The distance of the swipe. click Function A handler
 * triggered when a user just clicks on the item, rather than swipes it. If they
 * do not move, click is triggered, if they do move, it is not.
 * 
 * fingers int Default 1. The number of fingers to trigger the swipe, 1 or 2.
 * threshold int Default 75. The number of pixels that the user must move their
 * finger by before it is considered a swipe. triggerOnTouchEnd Boolean Default
 * true If true, the swipe events are triggered when the touch end event is
 * received (user releases finger). If false, it will be triggered on reaching
 * the threshold, and then cancel the touch event automatically. allowPageScroll
 * String Default "auto". How the browser handles page scrolls when the user is
 * swiping on a touchSwipe object. "auto" : all undefined swipes will cause the
 * page to scroll in that direction. "none" : the page will not scroll when user
 * swipes. "horizontal" : will force page to scroll on horizontal swipes.
 * "vertical" : will force page to scroll on vertical swipes.
 * 
 * This jQuery plugin will only run on devices running Mobile Webkit based
 * browsers (iOS 2.0+, android 2.2+)
 */
(function($) 
{
	...
	$.fn.swipe = function(options) 
	{
		...
		var defaults = {
			...
			swipe 			: null,		// Function - A catch all handler that
                                        // is triggered for all swipe
                                        // directions. Accepts 2 arguments, the
                                        // original event object and the
                                        // direction of the swipe : "left",
                                        // "right", "up", "down".
			swipeLeft		: null,		// Function - A handler that is
                                        // triggered for "left" swipes. Accepts
                                        // 3 arguments, the original event
                                        // object, the direction of the swipe :
                                        // "left", "right", "up", "down" and the
                                        // distance of the swipe.
			swipeRight		: null,		// Function - A handler that is
                                        // triggered for "right" swipes. Accepts
                                        // 3 arguments, the original event
                                        // object, the direction of the swipe :
                                        // "left", "right", "up", "down" and the
                                        // distance of the swipe.
			swipeUp			: null,		// Function - A handler that is
                                        // triggered for "up" swipes. Accepts 3
                                        // arguments, the original event object,
                                        // the direction of the swipe : "left",
                                        // "right", "up", "down" and the
                                        // distance of the swipe.
			swipeDown		: null,		// Function - A handler that is
                                        // triggered for "down" swipes. Accepts
                                        // 3 arguments, the original event
                                        // object, the direction of the swipe :
                                        // "left", "right", "up", "down" and the
                                        // distance of the swipe.
			swipeStatus		: null,		// Function - A handler triggered for
                                        // every phase of the swipe. Handler is
                                        // passed 4 arguments: event : The
                                        // original event object, phase:The
                                        // current swipe face, either "start�,
                                        // "move�, "end� or "cancel�. direction
                                        // : The swipe direction, either "up�,
                                        // "down�, "left " or "right�.distance :
                                        // The distance of the swipe.
			click			: null,		// Function - A handler triggered when a
                                        // user just clicks on the item, rather
                                        // than swipes it. If they do not move,
                                        // click is triggered, if they do move,
                                        // it is not.
			
			
			preventClick: false,
			triggerOnTouchEnd : true,	// Boolean, if true, the swipe events
                                        // are triggered when the touch end
                                        // event is received (user releases
                                        // finger). If false, it will be
                                        // triggered on reaching the threshold,
                                        // and then cancel the touch event
                                        // automatically.
			allowPageScroll : "auto" 	/*
                                         * How the browser handles page scrolls
                                         * when the user is swiping on a
                                         * touchSwipe object. "auto" : all
                                         * undefined swipes will cause the page
                                         * to scroll in that direction. "none" :
                                         * the page will not scroll when user
                                         * swipes. "horizontal" : will force
                                         * page to scroll on horizontal swipes.
		...
		return this.each(function() 
		{
			...
			var triggerElementID = null; 	// this variable is used to identity
                                            // the triggering element
			var fingerCount = 0;			// the current number of fingers
                                            // being used.
			...
			/**
             * Event handler for a touch start event. Stops the default click
             * event from triggering and stores where we touched
             */
			...
			/**
             * Event handler for a touch move event. If we change fingers during
             * move, then cancel the event
             */
			function touchMove(event) 
			{
				...
				// Check if we need to prevent default evnet (page scroll) or
                // not
				...
				if ( fingerCount == defaults.fingers ) 
				{
					...
					// If we trigger whilst dragging, not on touch end, then
                    // calculate now...
					if (!defaults.triggerOnTouchEnd)
					{
						// if the user swiped more than the minimum length,
			...
			/**
             * Event handler for a touch end event. Calculate the direction and
             * trigger events
             */
			function touchEnd(event) 
			{
				...
				{
					phase = PHASE_END;
					// check to see if more than one finger was used and that
                    // there is an ending coordinate
			...
			/**
             * Trigger the relevant event handler The handlers are passed the
             * original event, the element that was swiped, and in the case of
             * the catch all handler, the direction that was swiped, "left",
             * "right", "up", or "down"
             */
			...
			/**
             * Checks direction of the swipe and the value allowPageScroll to
             * see if we should allow or prevent the default behaviour from
             * occurring. This will essentially allow page scrolling or not when
             * the user is swiping on a touchSwipe object.
             */
			...
			/**
             * Calcualte the direction of the swipe This will also call
             * caluculateAngle to get the latest angle of swipe
             */
...
/*
 * ! jQuery UI Widget 1.9.2 http://jqueryui.com
 * 
...
/*
 * ! jQuery UI Core 1.9.2 http://jqueryui.com
 * 
 * Copyright 2012 jQuery Foundation and other contributors Released under the
 * MIT license. http://jquery.org/license
 * 
...
/*
 * ! jQuery UI Mouse 1.9.2 http://jqueryui.com
 * 
 * Copyright 2012 jQuery Foundation and other contributors Released under the
 * MIT license. http://jquery.org/license
 * 
...
/*
 * ! jQuery UI Slider 1.9.2 http://jqueryui.com
 * 
 * Copyright 2012 jQuery Foundation and other contributors Released under the
 * MIT license. http://jquery.org/license
 * 
...
/*
 * ! jQuery UI Datepicker 1.9.2 http://jqueryui.com
 * 
 * Copyright 2012 jQuery Foundation and other contributors Released under the
 * MIT license. http://jquery.org/license
 * 
		...
		$.fn.setFocus = function(opts){
			...
			if( !opts.fast ){
				...
				if( fxParent[0] ){
					...
					focusTimer = setTimeout( focusFn, minFocusTimer );// min
                                                                        // 54
				}
			} else {
				focusFn();
